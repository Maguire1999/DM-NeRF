...
from networks.evaluator import to8b, ins_eval, ConfusionMatrix, ins_eval_except
...
def render_test(position_embedder, view_embedder, model_coarse, model_fine, render_poses, hwk, args, gt_imgs=None,
    ...
    try:
        ...
        color_dict = gt_color_dict['scannet'][scene_name]
        if args.ins_num is not None:
            if len(color_dict) != args.ins_num + 1:
                for i in range(args.ins_num + 1):
                    color_dict[str(i)] = i
        ...
        if gt_imgs is not None:
            ...
            if crop_mask is not None:
                ...
                if valid_gt_num > 0:
                    ...
                    try:
                        pred_label, ap, pred_matched_order,iou_ins_eval = ins_eval(ins.cpu(), gt_ins, valid_gt_num, args.ins_num, mask)
                    except:
                        pred_label, ap, pred_matched_order, iou_ins_eval = ins_eval_except(ins.cpu(), gt_ins, valid_gt_num,
                                                                                    args.ins_num, mask)
                    # pred_label, ap, pred_matched_order, iou_ins_eval = ins_eval_except(ins.cpu(), gt_ins, valid_gt_num,
                    #                                                                 args.ins_num, mask)
                else:
                ...
                if valid_gt_num > 0:
                    pred_label, ap, pred_matched_order,iou_ins_eval = ins_eval(ins.cpu(), gt_ins, valid_gt_num, args.ins_num)
                else:
                    pred_label = -1 * torch.ones([H, W])
                    ap = torch.tensor([1.0])
            # get ins_map
            ...
            print("iou_ins_eval:")
            # try:
            #     train_cm = ConfusionMatrix(num_classes=valid_gt_num)
            #     metric_iou = train_cm.add_batch(np.array(gt_ins.cpu().numpy(), dtype=np.uint8),
            #                                     np.array(ins.cpu().numpy(), dtype=np.uint8),
            #                                     return_miou=True)
            # except:
            #     metric_iou = []
            # metric_ious.append(metric_iou)
            # print(f"metric_iou: {metric_iou}")
    ...
    if gt_imgs is not None:
        ...
        aps = np.array(aps)
        # try:
        #     metric_ious = np.array(metric_ious)

        # output = np.stack([psnrs, ssims, lpipses, aps[:, 0], aps[:, 1], aps[:, 2], aps[:, 3], aps[:, 4], aps[:, 5], metric_ious[:]])
        output = np.stack([psnrs, ssims, lpipses, aps[:, 0], aps[:, 1], aps[:, 2], aps[:, 3], aps[:, 4], aps[:, 5]])
        output = output.transpose([1, 0])
        out_ap = np.mean(aps, axis=0)
        # out_miou = np.mean(metric_ious, axis=0)
        # mean_output = np.array([np.mean(psnrs), np.mean(ssims), np.mean(lpipses),
        #                         out_ap[0], out_ap[1], out_ap[2], out_ap[3], out_ap[4], out_ap[5],out_miou])
        # mean_output = mean_output.reshape([1, 10])
        ...
        # print("miou: {:.4f}",out_miou)



def render_test_scannet(position_embedder, view_embedder, model_coarse, model_fine, render_poses, hwk, args, gt_imgs=None,
                gt_labels=None, ins_rgbs=None, savedir=None, matched_file=None, crop_mask=None):
    data_info = args.datadir.split('/')
    dataset_name = data_info[2]
    scene_name = data_info[-1]
    ...
    for i, c2w in enumerate(render_poses):
        ...
        if gt_imgs is not None:
            ...
            if crop_mask is not None:
                ...
                if valid_gt_num > 0:
                    mask = (gt_label < args.ins_num).type(torch.float32)
                    pred_label = torch.argmax(ins, dim=-1)
                    # pred_label, ap, pred_matched_order,iou_ins_eval = ins_eval(ins.cpu(), gt_ins, valid_gt_num, args.ins_num, mask)
                else:
                    pred_label = -1 * torch.ones([H, W])
                    ap = torch.tensor([1.0])
        ...
        if savedir is not None:
            rgb8 = to8b(rgb.cpu().numpy())
            # ins_img = render_label2img(pred_label, ins_rgbs, color_dict, ins_map)
            ins_img = render_gt_label2img(pred_label, ins_rgbs, color_dict)
            filename = os.path.join(savedir, '{:03d}.png'.format(i))
            fileins = os.path.join(savedir, f"instance_{str(i).zfill(3)}.png")
            # cv2.imwrite(fileins, ins_img)
            pred_ins_file = os.path.join(savedir, f'{i}_ins_pred_mask.png')
